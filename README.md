...
※チャットボットのスコープは上記5まで（初回ヒアリング受信〜有料オファー送信）
...
graph TD
   ...
   subgraph チャットボット["🤖 チャットボット処理範囲"]
      ...
      D -->|有効| F["(チャットボット) 深堀りヒアリング送信"]
      D -->|無効| G["(チャットボット) エラーメッセージ送信"]
      F --> H{"(ユーザー) 深堀り回答？"}
      H -->|No| I["(チャットボット) リマインダー or 終了"]
      H -->|Yes| J{"(チャットボット) 回答内容チェック"}
      J -->|有効| K["(チャットボット) ChatGPT APIリクエスト"]
      J -->|無効| L["(チャットボット) エラーメッセージ送信"]
      L --> H
      K --> M["(チャットボット) 占い結果生成"]
      M --> N["(チャットボット) LINEに返信"]
      N --> O["(チャットボット) 有料オファー送信"]
   end       
   O --> P["(チャットボット) 終了"]
   O --> Q{"(ユーザー) 有料オファー承諾？"}
   Q -->|No| R["フロー終了"]
   Q -->|Yes| S["(運営者) 有料ヒアリング項目送信"]
   S --> T{"(ユーザー) 詳細回答？"}
   T -->|Yes| U["(運営者) ChatGPT APIリクエスト"]
   T -->|No| V["フロー終了"]
   U --> W["(運営者) 最終占い結果生成"]
   W --> X["(運営者) LINEに返信"]
   X --> Y["フロー完了"]
...
- WAITING_FOLLOWUP_ANSWER: 深堀りヒアリング回答待ち
